...
def legendre_approximation(n: int, ap: float):
    ...
    poles = [pole for pole in 1j * poly.roots if pole.real < 0]
    for pole in poles:
        gain *= pole
...
def legendre_odd_integrated_polynomial(n: int):
    ...
    for i in range(1, k):
        ai = a0 * (2*i + 1)
        new_poly = ai * legendre_polynomial(i)
        poly = polyadd(poly, new_poly)
